...
model = ifcopenshell.open('C:/Users/Dell/Desktop/DOCS/EXTRACAO-IFC/modelopilar1.ifc')
...
for pilar in model.by_type ('IfcColumn', 1):
	...
	area_z = round ((ifcopenshell.util.shape.get_side_area(shape.geometry, 'Z', angle = 90.0)),2)
	print (f'Area Z : {area_z:.2f} m2')

	area_y = round((ifcopenshell.util.shape.get_side_area(shape.geometry, 'Y', angle = 90.0)),2)
	print (f'Area Y : {area_y:.2f} m2')

	area_x = round((ifcopenshell.util.shape.get_side_area(shape.geometry, 'X', angle = 90.0)),2)
	print (f'Area X : {area_x:.2f} m2')



#print (f'Total de pilares no modelo', len(column_type))
